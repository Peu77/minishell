...
/*   Updated: 2025/01/14 14:00:06 by eebert           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool match_files(const char *pattern, t_list_data files, t_list **matches, size_t *matches_count) {
    char *tmp_str;
    t_list *new_node;

    while (*files.list) {
        if ((*(char *) (*files.list)->content) == '.' && *pattern != '.') {
...
bool expand_wildcard(const char *old_pattern, size_t pattern_len, t_list **list) {
    ...
    matches = 0;
    if (!match_files(expanded_pattern, (t_list_data){
                         .list = &files,
                         .count = file_count
                     }, list, &matches))
        return (ft_lstclear(&files, free), false);
