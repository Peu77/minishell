...
/*   Updated: 2025/01/27 16:37:58 by eebert           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*find_command_in_path(const char *command)
{
	...
	if (!env_path)
		return (NULL);
	paths = gc_add(ft_split(env_path, ':'));
	while (paths[++i])
	{
		gc_add(paths[i]);
...
int	get_path(t_command **command)
{
	...
	if (ft_strncmp((*command)->command_name, "./", 2) == 0
		|| (*command)->command_name[0] == 0
		|| ft_strchr((*command)->command_name, '/') != NULL)
	{
