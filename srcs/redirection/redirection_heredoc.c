...
/*   Updated: 2025/01/18 21:33:25 by ftapponn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void handle_heredoc_input(int temp_fd, const char *delimiter, t_command *command, t_parenthesis_fd *parenthesis_fd)
{
    char *buffer;
    ...
    else
    {
        ...
        if (WIFEXITED(status))
        {
            if (WEXITSTATUS(status) != 0)
                exit(pec("Heredoc process failed\n"));
        }
        else
            exit(pec("Heredoc process terminated abnormally\n"));
    }
